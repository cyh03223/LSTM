...
class JordanLSTMCell(tf.keras.layers.Layer):
    def __init__(self, units):
        ...
        self.state_size = [self.units, self.units]  # [estimated_state, cell_state]
    
    def build(self, input_shape):
        input_dim = input_shape[-1]
        
        # Jordan estimated state weights (W_f_x, W_i_x, W_cbar_x, W_o_x) for all 4 gates
        self.W_estimate = self.add_weight(shape=(self.units, 4 * self.units),
...
single_cell = JordanLSTMCell(units)
mutiple_cells = [
    JordanLSTMCell(units),
    JordanLSTMCell(units),
]
